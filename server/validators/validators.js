...
const mergeOptions = (into, from) =>
  existsAndIsObject(from) ? Object.assign(into, from) : into;
...
const existResourceById = (field, model, options) => {
  let defaultObject = {
    options: (value, { req, location, path }) => {
      return model.findByPk(value).then((result) => {
...
const isInEnumList = (field, enumList, options) => {
  let defaultObject = {
    errorMessage: isNotInEnumListErrorMessage(field, enumList),
    options: (value, { req, location, path }) => {
      return enumList.find((el) => el === value);
    },
  };
